...
def compute_mapped_landmarks(ws_source, ws_target) -> dict:
    ...
    Algorithm:
      1. Establish ws_target's local coordinate frame:
           origin  = PCA centroid of all placed points
           axis    = tilt-aware face direction (from _face_axis)
           scale   = face height H_t (chin-to-brow reference length)
      2. Express each target landmark in that normalised local frame:
           along  = projection onto face axis   / H_t
           perp   = projection onto perp axis   / H_t
      3. Reconstruct in ws_source's frame (same formula, H_s scale):
           mapped = source_origin + along * H_s * source_axis
                                  + perp  * H_s * source_perp

    This keeps ws_source's overall face position and size while
    replacing its internal structure with ws_target's proportions.
    Head tilt of either face is fully accounted for.
    """
    ...
    # Right-perpendicular direction for each frame
    # (rotate axis 90° clockwise in screen coords: dy, -dx)
    ps_x, ps_y =  ds_y, -ds_x
    pt_x, pt_y =  dt_y, -dt_x
    ...
    for key in lm_t:
        ...
        # Express target point in target's normalised local coords
        vx = tx - ct_x
        vy = ty - ct_y
        along = (vx * dt_x + vy * dt_y) / H_t
        perp  = (vx * pt_x + vy * pt_y) / H_t

        # Reconstruct in source frame scaled by H_s
