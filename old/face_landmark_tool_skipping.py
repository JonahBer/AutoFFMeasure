...
class FaceLandmarkApp(tk.Tk):
    ...
    def _compute_estimated_points(self) -> int:
        """
        For each skipped or missing landmark:
          1. If bilateral: mirror the opposite side across the face midline
          2. If single: estimate from neighbouring available points
        ...
        midline = _face_midline(ws.landmarks)

        # ── Pass 1: mirror bilateral skipped points ───────────────────
        for key in list(ws.skipped_keys):
            if key in ws.landmarks:
                continue           # already filled (e.g. from a previous load)
...
def _face_midline(landmarks: dict) -> Optional[float]:
    """Estimate the face vertical midline X from available centre-line points."""
    centre_keys = ["glabella", "nose_bottom_middle", "lips_center_meet",
                   "mouth_upper_low_u", "chin_bottom_apex"]
...
def _estimate_singles(lm: dict, est: set):
    """
    Attempt to fill in missing single-centre landmarks using neighbours.
    Modifies lm and est in-place.
    """
    def _mid(*keys):
        pts = [lm[k] for k in keys if k in lm]
        if not pts: return None
        return (int(sum(p[0] for p in pts)/len(pts)),
                int(sum(p[1] for p in pts)/len(pts)))

    # glabella — midpoint between inner brows
    if "glabella" not in lm:
        p = _mid("eyebrow_inside_L", "eyebrow_inside_R")
        if p: lm["glabella"] = p; est.add("glabella")

    # nose_bottom_middle — midpoint of alar bases or nostrils
    ...
    # mouth_upper_low_u — midpoint of philtrum peaks
    ...
    # lips_center_meet — midpoint between outer lip creases at their avg y
    if "lips_center_meet" not in lm:
        p = _mid("lips_outer_crease_L", "lips_outer_crease_R",
                 "mouth_upper_low_u")  # mouth_upper_low_u may have just been estimated
        if p: lm["lips_center_meet"] = p; est.add("lips_center_meet")

    # lip_bottom_center — midpoint of under-mouth apexes
    ...
    # chin_bottom_apex — midpoint between chin outer sides, pushed down slightly
...
def _effective_landmarks(ws: "Workspace") -> dict:
    """
    Return a fully-populated landmarks dict for metric computation.
    Runs estimation on a copy without modifying the workspace.
    """
    lm  = dict(ws.landmarks)
    est = set(ws.estimated_keys)
    mid = _face_midline(lm)

    # Mirror any bilateral keys still missing (handles partial sessions)
    all_keys = set(p["key"] for p in PROMPTS)
    for key in all_keys:
        if key in lm: continue
        mk = _mirror_key(key)
        if mk and mk in lm and mid is not None:
            ox, oy = lm[mk]
            lm[key] = (int(2 * mid - ox), oy)
            est.add(key)
