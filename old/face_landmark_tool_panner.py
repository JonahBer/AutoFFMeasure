...
class FaceLandmarkApp(tk.Tk):
    ...
    MIN_WIDTH  = 960
    MIN_HEIGHT = 700
    PAN_SPEED  = 30   # pixels per scroll tick for panning
    ...
    def _build_ui(self):
        ...
        self.canvas.bind("<Control-MouseWheel>",   self._on_ctrl_scroll)      # Windows
        self.canvas.bind("<Control-Button-4>",     self._on_ctrl_scroll)      # Linux scroll up
        self.canvas.bind("<Control-Button-5>",     self._on_ctrl_scroll)      # Linux scroll down
        self.canvas.bind("<MouseWheel>",           self._on_pan_vertical)     # Windows vertical
        self.canvas.bind("<Button-4>",             self._on_pan_vertical)     # Linux scroll up
        self.canvas.bind("<Button-5>",             self._on_pan_vertical)     # Linux scroll down
        self.canvas.bind("<Shift-MouseWheel>",     self._on_pan_horizontal)   # Windows horizontal
        self.canvas.bind("<Shift-Button-4>",       self._on_pan_horizontal)   # Linux horizontal
        self.canvas.bind("<Shift-Button-5>",       self._on_pan_horizontal)   # Linux horizontal
    ...
    def refresh_display(self, new_scale: float = None):
        ...
        if new_scale is not None:
            ...
            # Update combobox label to show current %
    ...
    # ------------------------------------------------------------------
    # Scroll pan  (plain scroll = vertical, Shift+scroll = horizontal)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Ctrl + scroll zoom  (zooms toward the mouse cursor)
    # ------------------------------------------------------------------

    ZOOM_STEP    = 1.15   # multiply / divide scale by this per scroll tick
    ZOOM_MIN     = 0.05   # 5 %
    ZOOM_MAX     = 8.0    # 800 %
    ...
    def _on_ctrl_scroll(self, event):
        ...
        # Determine scroll direction (Windows uses event.delta; Linux uses num)
        if event.num == 4:
            direction = 1
        elif event.num == 5:
            direction = -1
        ...
        new_scale = old_scale * (self.ZOOM_STEP if direction > 0 else 1 / self.ZOOM_STEP)
        new_scale = max(self.ZOOM_MIN, min(self.ZOOM_MAX, new_scale))

        if abs(new_scale - old_scale) < 1e-9:
            return

        # ── Anchor point: image coords under the mouse ──────────────
        # Canvas coords of the mouse (accounting for current scroll position)
        mouse_cx = self.canvas.canvasx(event.x)
        mouse_cy = self.canvas.canvasy(event.y)

        # Corresponding position in original-image space
        img_x = mouse_cx / old_scale
        img_y = mouse_cy / old_scale

        # ── Redraw at new scale ──────────────────────────────────────
        self.refresh_display(new_scale=new_scale)

        iw, ih = self.original_image.size
        new_canvas_w = max(1, int(iw * new_scale))
        new_canvas_h = max(1, int(ih * new_scale))

        # ── Scroll so the same image point stays under the cursor ────
        # The target canvas coord of img_x/img_y after the zoom:
        target_cx = img_x * new_scale
        target_cy = img_y * new_scale

        # Fraction of total canvas width/height that should be at the left/top edge:
        frac_x = (target_cx - event.x) / new_canvas_w
        frac_y = (target_cy - event.y) / new_canvas_h

        self.canvas.xview_moveto(max(0.0, frac_x))
        self.canvas.yview_moveto(max(0.0, frac_y))
