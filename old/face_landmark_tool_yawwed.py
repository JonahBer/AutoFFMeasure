...
def _face_axis(landmarks: dict):
    ...
    Compute the face's central axis as (cx, cy, dx, dy):
      (cx, cy) — centroid of TRUE centre-line anatomical points only
      (dx, dy) — unit direction vector along the axis (pointing downward)

    Centroid uses only known midline points so wide bilateral pairs
    (jaw, cheeks) cannot pull it sideways.
    PCA for tilt uses midline points + bilateral midpoints combined.
    """
    # ── Centroid: TRUE midline points only ────────────────────────────
    MIDLINE_KEYS = [
        "glabella",
        "nose_bottom_middle",
        "mouth_upper_low_u",
    ...
    centre_pts = [landmarks[k] for k in MIDLINE_KEYS if k in landmarks]

    # Fallback: midpoints of narrow bilateral pairs that lie near the axis
    NEAR_AXIS_PAIRS = [
    ...
    near_mids = []
    for lk, rk in NEAR_AXIS_PAIRS:
        if lk in landmarks and rk in landmarks:
            lp, rp = landmarks[lk], landmarks[rk]
            near_mids.append(((lp[0]+rp[0])/2, (lp[1]+rp[1])/2))

    centroid_pts = centre_pts if centre_pts else near_mids
    if not centroid_pts:
        return None

    cx = sum(p[0] for p in centroid_pts) / len(centroid_pts)
    cy = sum(p[1] for p in centroid_pts) / len(centroid_pts)

    # ── Tilt (PCA): use centroid pts + near-axis bilateral midpoints ──
    pca_pts = centroid_pts + near_mids
    if len(pca_pts) < 2:
        return (cx, cy, 0.0, 1.0)

    sxx = sum((p[0]-cx)**2 for p in pca_pts)
    syy = sum((p[1]-cy)**2 for p in pca_pts)
    sxy = sum((p[0]-cx)*(p[1]-cy) for p in pca_pts)
    ...
    # Ensure axis points downward (screen y increases downward)
    if dy < 0:
        dx, dy = -dx, -dy
...
def compute_mapped_landmarks(ws_source, ws_target) -> dict:
    ...
    Algorithm:
      1.  Build each face's coordinate frame:
              origin  = centroid of TRUE midline anatomical points
              axis    = tilt-corrected face direction (PCA of midline+near-axis pairs)
              H       = face height (chin-to-brow reference)
      2.  Detect face yaw from bilateral asymmetry:
              r = median(d_L / d_R)  for all bilateral pairs
              r = 1.0 → frontal; r ≠ 1.0 → face is rotated
          Because a symmetric face has equal L/R half-widths, any deviation from
          r = 1.0 is due to foreshortening from the horizontal rotation angle.
      3.  De-yaw the target's perp components using the geometric-mean correction:
              left  side (perp < 0):  perp_true = perp * (1 / sqrt(r_t))
              right side (perp > 0):  perp_true = perp * sqrt(r_t)
          Both sides now equal D/H (the symmetric proportional distance).
      4.  Re-yaw in source's frame to reproduce source's apparent rotation:
              left  side: perp_source = perp_true * sqrt(r_s)
              right side: perp_source = perp_true / sqrt(r_s)
          Combined single factors:
              left  side: multiply perp by  sqrt(r_s / r_t)
              right side: multiply perp by  sqrt(r_t / r_s)
      5.  Reconstruct:
              mapped = source_origin + along * H_s * source_axis
                                     + perp_corrected * H_s * source_perp
    """
    ...
    ps_x, ps_y = _make_perp(ds_x, ds_y, lm_s, cs_x, cs_y)
    pt_x, pt_y = _make_perp(dt_x, dt_y, lm_t, ct_x, ct_y)

    # ── Detect yaw foreshortening for each face ───────────────────────
    r_s = _bilateral_half_ratio(lm_s, cs_x)
    r_t = _bilateral_half_ratio(lm_t, ct_x)

    # Clamp to avoid extreme corrections on very partial landmark sets
    r_s = max(0.5, min(2.0, r_s))
    r_t = max(0.5, min(2.0, r_t))

    # Combined per-side scale factors (de-yaw target + re-yaw source)
    # Left side (perp < 0):  scale = sqrt(r_s) / sqrt(r_t) = sqrt(r_s / r_t)
    # Right side (perp >= 0): scale = sqrt(r_t) / sqrt(r_s) = sqrt(r_t / r_s)
    eps = 1e-9
    ...
    for key in lm_t:
        ...
        # Express target point in normalised local coords
        vx = tx - ct_x
        vy = ty - ct_y
        along = (vx * dt_x + vy * dt_y) / H_t
        perp  = (vx * pt_x + vy * pt_y) / H_t

        # Apply foreshortening correction (de-yaw target, re-yaw source)
        perp_corrected = perp * (scale_left if perp < 0 else scale_right)

        # Reconstruct in source frame
