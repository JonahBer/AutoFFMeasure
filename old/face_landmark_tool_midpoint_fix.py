...
def compute_mapped_landmarks(ws_source, ws_target) -> dict:
    ...
    Algorithm:
      1. Establish each face's local coordinate frame:
           origin  = centroid of TRUE midline points (centroid from _face_axis)
           axis    = tilt-aware face direction (from _face_axis)
           H       = face height (chin-to-brow, as returned by _ref_length)
      2. Perpendicular direction is chosen so that a known right-of-axis
         landmark (e.g. nose_nostril_outside_R, eye_outside_corner_R) has
         a POSITIVE perpendicular component — ensures L/R sign is consistent
         between source and target frames.
      3. Express each target landmark in normalised local coords:
           along = dot(v, axis) / H_t
           perp  = dot(v, perp) / H_t
      4. Reconstruct in source frame:
           mapped = source_origin + along * H_s * source_axis
                                  + perp  * H_s * source_perp
    """
    ...
    def _make_perp(dx, dy, lm, cx, cy):
        """
        Return perpendicular unit vector (px, py) such that a landmark
        that should be to the RIGHT of the axis (anatomically) has
        positive perp component.  Tries a known right-side key list.
        Fallback: rotate axis 90° CW (dy, -dx).
        """
        px, py = dy, -dx   # 90° clockwise rotation
        # Check sign with a right-side landmark
        RIGHT_ANCHOR_KEYS = [
        ...
        for rk in RIGHT_ANCHOR_KEYS:
            if rk in lm:
                ...
                dot = vx * px + vy * py
                if dot < 0:
                    px, py = -px, -py   # flip so right-side → positive
                break
        return px, py
